...
/* 
 * page table entry: 19 bits, padded to 4B
 * 256 entries x 4 B = 1024 B = exactly one frame.
 */
typedef struct {
    uint32_t present    : 1;            /* same as valid/invalid */
    uint32_t frame      : FRAME_BITS;   /* valid only when present=1 */
    uint32_t referenced : 1;            /* the Accessed bit; set by the walker */
    uint32_t dirty      : 1;            /* set on a store; vs DISK, not cache  */
    uint32_t prot       : 3;            /* PROTECTED* */
...
typedef struct {
    uint16_t   pid;
    PageTable *pt;              /* malloc'd, NULL means it is not allocated */
    uint32_t   pt_frame;        /* the frame holding the page table */
